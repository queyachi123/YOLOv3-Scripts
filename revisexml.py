...
@author: zhangyue
change some contents of .xml label"""
    ...
    if (os.path.exists(path)):
        ...
        for file in f : 
            ...
            for filename in root.iter('filename'):
                new = filename.text.replace('.png','')
                tree.find('filename').text = new
                print(tree.find('filename').text)
                
            for path in root.iter('path'):
                new = path.text.replace(r'/home/gpu_server2/SSD_data/traffic_img/201606061616/2', \
                                        r'D:\Research\traffic_img\201606061616\2')
                #python中字符串的最后一个字符是斜杠会导致出错：SyntaxError: EOL while scanning string literal
                tree.find('path').text = new
                print (tree.find('path').text)
                save_path = new.replace('png','xml')
...
if __name__ == "__main__":
    traversalDir_XMLFile('D:\\Research\\traffic_label\\201606061616')
    '''
    #1. 读取xml文件  
